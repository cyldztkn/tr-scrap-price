...
const priceController = {
    ...
    async getHistoricalPricesByCompany(req, res, next) {
        try {
            const companyName = req.params.company;
            const currency = req.query.currency?.toUpperCase() || 'TRY';
            const history = await priceService.getHistoricalPricesByCompany(companyName, currency);
            res.json(history);
        } catch (error) {
            next(error);
        }
    ...
    async getCategoryPriceAnalysis(req, res, next) {
        try {
            ...
            const currency = req.query.currency?.toUpperCase() || 'TRY';
            const analysis = await priceService.getCategoryPriceAnalysis(category, currency);
            res.json(analysis);
        } catch (error) {
            next(error);
        }
